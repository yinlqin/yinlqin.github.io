...
layui.use('table', function(){
    ...
    table.render({
      ...
      ,title: 'qf_wuliaoxinxi'
      ,cols: [[
        {type: 'checkbox', fixed: 'left'}
        ,{field:'id', title:'物料编码', width:120, fixed: 'left', unresize: true, sort: true}
        ,{field:'wu_liao_lei_bie', title:'物料类别	', width:120, edit: 'text', sort: true}
        ,{field:'wu_liao_ming_cheng', title:'物料名称	', width:150, edit: 'text', templet: function(res){
          return '<em>'+ res.email +'</em>'
        }}
        ,{field:'wu_liao_pin_pai', title:'品牌', width:80, edit: 'text'}
        ,{field:'gui_ge_xing_hao', title:'规格型号	', width:150}
        ,{field:'dan_wei', title:'单位'}
        ,{field:'cai_zhi', title:'材质', width:100}
        ,{field:'color', title:'颜色', width:100}
        ,{field:'bei_zhu', title:'备注', width:100, sort: true}
        ,{field:'joinTime', title:'库位', width:100, sort: true}
        ,{fixed: 'ku_wei', title:'操作', toolbar: '#barDemo', width:150}
      ]]
      ,page: true
    });
...
$('#parentIframe_xinjianwuliao').on('click', function(){
    layer.open({
      type: 2,
      title: '新建物料',
      shadeClose: true, //点击遮罩关闭层
      area : ['1000px' , '820px'],
      content: '../admin/qf_jcxxgl_wuliaoguanli_new.php'
    });
});
...
$("#test110").on('click',function(){
    layer.msg("hello layer");
});


  //物料查询界面的添加物料按钮
$('#wuliaochaxun_xinjianwuliao').on('click', function(){
    layer.open({
        type: 2,//type为2表示为iframe层
        title: '新建物料',//弹出层名称
        area : ['40%' , '80%'],//宽高
        content: '../admin/qf_jcxxgl_wuliaoguanli_new.php',//弹出层页面地址
...
//     var that = this;//避免this的重复
//     layer.open({//一个弹出框
//         title:"物料信息删除",//弹出框名称
//         btn:['确认','取消'],//页面上两个按钮
//         content:'确认删除？',//写了地址会跳转到地址页，如果写了文字会显示出来，
...
// })
//入库管理界面的删除按钮
$('#rukuguanli_shanchu').on('click', function(){
    layer.open({
        type: 2,//type为2表示为iframe层
        title: '删除入库单',//弹出层名称
        area : ['40%' , '80%'],//宽高
        content: '../admin/qf_rk_rukudan_add.php',//弹出层页面地址
        end:function()//弹出层关闭之后的操作
        {
            location.reload();//父页面的重新加载
        }
